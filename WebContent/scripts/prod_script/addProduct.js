...
function loadImage(input, num) {
	console.log("선택한 파일들", input.files);
	let files = input.files;
	
	if($("#detail-imgbox"+num).children('img').length> 0) {
		$("#detail-imgbox"+num).children('img').remove();
	}
	...
	newImage.style.margin = "5px 5px";
	newImage.style.objectFit = "contain";
	newImage.style.marginRight = "20px";

	// 이미지를 image-show div에 추가
	var container = document.getElementById("detail-imgbox"+num);
